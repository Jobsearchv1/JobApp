...
const Workspace = () => {
  ...
  const fetchJobs = async () => {
    setLoading(true);
    try {
      const response = await axios.get('http://localhost:3032/api/jobs');
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this job?')) {
      try {
        await axios.delete(`http://localhost:3032/api/jobs/${id}`);
        fetchJobs();
      } catch (error) {
  ...
  const handleUpdateJob = async () => {
    try {
      await axios.put(`http://localhost:3032/api/jobs/${selectedJob.id}`, {
        title: jobTitle,
    ...
    <Container className="mt-5">
      ...
      <Row className="mb-5">
        <Col md={6} lg={4} className="mb-4">
          <Card className="workspace-card border-primary">
        ...
        <Col md={6} lg={4} className="mb-4">
          <Card className="workspace-card border-success">
        ...
        <Col md={12} lg={4} className="mb-4">
          <Card className="workspace-card border-info">
            ...
            <>
              ...
              <Pagination>
                {[...Array(Math.ceil(jobs.length / jobsPerPage)).keys()].map(num => (
                  <Pagination.Item key={num + 1} onClick={() => paginate(num + 1)}>
                    {num + 1}
                  </Pagination.Item>
                ))}
              </Pagination>
